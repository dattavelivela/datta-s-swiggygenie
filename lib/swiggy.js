...
export function getToolData(result) {
...
export function getCartTotal(cart) {
  const candidates = [
    ...
    cart?.cartTotal,
    cart?.bill?.total,
    cart?.billDetails?.total,
    cart?.cart?.total,
    cart?.cart?.totalAmount
  ];
  const value = candidates.find((item) => typeof item === "number" || typeof item === "string");
  return Number(value || 0);
}

export function getArrayFromToolData(data, keys) {
...
export function isRetryableMcpError(error) {
  const status = error?.status || error?.response?.status;
  if (status >= 500 && status < 600) return true;

  const message = String(error?.message || "");
  return ["UPSTREAM_TIMEOUT", "UPSTREAM_ERROR", "INTERNAL_ERROR"].some((code) => message.includes(code));
}
